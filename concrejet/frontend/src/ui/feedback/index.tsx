...
export function IndustrialAlert({
  ...
  return (
    <div className={`industrial-alert ${variant}`} role="alert">
      {icon} {children}
    </div>
  );
}
...
export function OfflineIndicator({ online, pending }: { online: boolean; pending: number }) {
  return (
    <span className={`sync-pill ${online ? 'ok' : 'atencao'}`}>
      {online ? '✓ Online' : `▲ Offline · ${pending} pendentes`}
    </span>
  );
}
...
export function SyncIndicator({
  ...
  state: 'sincronizado' | 'pendente' | 'enviando' | 'erro';
  pending: number;
}) {
  const text =
    state === 'sincronizado'
  ...
  return (
    <span
      className={`sync-pill ${state === 'erro' ? 'parada' : state === 'sincronizado' ? 'ok' : 'atencao'}`}
    >
      {text}
    </span>
  );
